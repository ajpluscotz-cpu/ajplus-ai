/* ═══════════════════════════════════════════════════
   AJPLUS AI — app.js (v3 FINAL)
   js/app.js — functions za msingi tu
═══════════════════════════════════════════════════ */
...
function appendMsg(role, text) {
  ...
  if (role === 'ai') {
    ...
    av.style.cssText = 'background:var(--gold);display:flex;align-items:center;justify-content:center;color:#fff;font-size:.7rem;font-weight:700;border-radius:50%';
    const u = JSON.parse(localStorage.getItem('ajplus-user') || '{}');
    av.textContent = (u.name || 'W')[0].toUpperCase();
  }
...
function showTyping() {
  ...
  wrap.innerHTML = `
    ...
    <div class="msg-bub" style="display:flex;gap:4px;align-items:center;padding:12px 14px">
